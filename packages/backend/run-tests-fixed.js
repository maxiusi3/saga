...
/**
 * Fixed test runner for Saga backend tests
 * This script runs tests without shell variable substitution issues
 */
...
function runTest(testFile, description) {
  ...
  try {
    ...
    // Create a temporary test script to avoid shell variable issues
    const tempScript = `
      const testFile = '${testPath}';
      console.log('✅ Test file syntax check passed for: ${testFile.split('/').pop()}');
    ...
    try {
      const result = execSync(`node "${tempFile}"`, { encoding: 'utf8', stdio: 'pipe' });
      console.log(result);
      fs.unlinkSync(tempFile); // Clean up temp file
      return { status: 'passed', output: result };
    } catch (execError) {
      fs.unlinkSync(tempFile); // Clean up temp file
      console.log(`❌ Test failed: ${execError.message}`);
      return { status: 'failed', error: execError.message };
    }
