...
export function StoryRecommendations({ 
    ...
    return (
      <div className={`space-y-4 ${className}`}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900">Recommended Stories</h3>
        </div>
  ...
  if (recommendations.length === 0) {
    return (
      <div className={`${className}`}>
        <h3 className="text-lg font-medium text-gray-900 mb-4">Recommended Stories</h3>
          ...
          >
            <div className="p-4">
              ...
              <div className="flex items-center space-x-4 text-xs text-gray-500 mb-3">
                {recommendation.story.audioDuration && (
                  <div className="flex items-center">
                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {Math.round(recommendation.story.audioDuration / 60)} min
                  </div>
                )}
                <span>{formatRelativeTime(recommendation.story.createdAt)}</span>
              </div>
              ...
              <Link
                ...
                className="block w-full text-center bg-primary-600 text-white text-sm font-medium py-2 px-4 rounded hover:bg-primary-700 transition-colors"
              >
                Listen & Interact
              </Link>
            </div>
